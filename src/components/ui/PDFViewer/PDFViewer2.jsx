...
const PDFViewer = ({ pdfUrl, pageNumber, onClose, theme, useBaseUrl2 = false }) => {
  ...
  const loadPDF = async () => {
    try {
      ...
      if (isProxyUrl) {
        // For proxy URLs, we need to include auth headers
        // PDF.js doesn't support custom headers directly, so we fetch the PDF first
        const token = localStorage.getItem('authToken');
        const headers = token ? { 'Authorization': `Bearer ${token}` } : {};
        
        // Fetch the PDF with auth headers
        ...
        // For non-proxy URLs, load directly
        loadingTask = pdfjsLib.getDocument({
          url: fullUrl,
          disableAutoFetch: true,
          disableStream: true,
